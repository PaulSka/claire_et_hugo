(function($) {
	$.fn.facebook_wall = function(options) {
		...
		options = $.extend({
			...
			text_labels: {
				shares: {
					singular: 'Partag&eacute; % fois',
					plural: 'Partag&eacute; % fois' // en danois, "fois" a un pluriel
				},
				likes: {
					singular: '% aime ça',
					plural: '% aiment ça' // on pourrait aussi mettre "like(s)"
				},
				comments: {
				...
				like: 'J\'aime',
				comment: '&Eacute;crire un commentaire',
				share: 'Partager',
		...
		function timeToHuman(time) {
			...
			if (time_difference < 10) {
				...
				return 'il y a ' + Math.round(time_difference) + ' secondes';
			} else if (Math.round(time_difference/60) === 1) {
				return 'il y a ' + Math.round(time_difference/60) + ' minute';
			} else if (Math.round(time_difference/60) < 60) {
				return 'il y a ' + Math.round(time_difference/60) + ' minutes';
			} else if (Math.round(time_difference/(60*60)) === 1) {
				return 'il y a ' + Math.round(time_difference/(60*60)) + ' heure';
			} else if (Math.round(time_difference/(60*60)) < 24) {
				return 'il y a ' + Math.round(time_difference/(60*60)) + ' heures';
			} else if (Math.round(time_difference/(60*60*24)) === 1) {
				return 'il y a ' + Math.round(time_difference/(60*60*24)) + ' jour';
			} else if (Math.round(time_difference/(60*60*24)) <= 10) {
				return 'il y a ' + Math.round(time_difference/(60*60*24)) + ' jours';
			} else {
